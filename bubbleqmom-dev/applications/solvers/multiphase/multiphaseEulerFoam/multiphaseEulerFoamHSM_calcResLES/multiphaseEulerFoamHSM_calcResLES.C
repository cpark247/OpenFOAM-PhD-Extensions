...
{
    Info<< "hsm solver with time averaging" << endl;
        ...
        forAll(phases, phasei)
        {
            ...
            if (phases[phasei].name() == "air")
            {
                ...
                volScalarField term1 =
                    (one_minus_alpha_phase*alphaMax) / ((1.0 - alphaMax)*alpha_air_safe);
                volScalarField term2 = alphaMin/alpha_air_safe;

                CBeta = min(max(term1, term2), dimensionedScalar("one", dimless, 1.0));
            ...
            else // water phase
            {
                ...
                // Modelled fields for water
                nuMod_[phasei] = mesh.lookupObject<volScalarField>(nutFieldName); // nut
                volScalarField denominator_k = C_k*delta_safe;
                kMod_[phasei]       = sqr(nuMod_[phasei]/denominator_k); // (nut/(Ck*delta))^2
                epsilonMod_[phasei] = 2*nuMod_[phasei]*Foam::magSqr(Sres_phase);
                //epsilonMod_[phasei] = C_eps*pow(max(kMod_[phasei], dimensionedScalar("minK", dimVelocity*dimVelocity, SMALL_K)), 1.5)/delta_safe;
                ...
                volScalarField epsTotMean_safe = max(epsilonTotal_Mean_[phasei], dimensionedScalar("minEps", dimVelocity*dimVelocity/dimTime, 1e-12));
                ...
                //Info<<"deltaByKolmog: max = " << max(deltaByKolmog).value() << ", average = " << average(deltaByKolmog).value() << endl;
