...
int main(int argc, char *argv[])
{
    ...
    while (pimple.run(runTime))
    {
        ...
        Info<<"p min "<<min(p).value()<<" max "<<max(p).value()<<"\n";
        forAll(fluid.phases(),phasei)
        {
            volVectorField& tmpU = fluid.mesh().lookupObjectRef<volVectorField>(IOobject::groupName("U", fluid.phases()[phasei].name()));
            Info<<"U"<<fluid.phases()[phasei].name()<<" min "<<min(mag(tmpU)).value()<<" max "<<max(mag(tmpU)).value()<<"\n";
        }
