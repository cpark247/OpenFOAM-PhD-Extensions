...
{
    defineTypeNameAndDebug(LaakkonenMom, 0);
...
Foam::diameterModels::breakupMomModels::LaakkonenMom::
LaakkonenMom
(
...
void
Foam::diameterModels::breakupMomModels::LaakkonenMom::setBreakupRate
(
    ...
    breakupRate =
       ...
       *erfc
        (
            sqrt
            (
                C2_*popBal_.sigmaWithContinuousPhase(popBal_.refPhase())
               /(
                    continuousPhase.rho()*pow(di, 5.0/3.0)
                   *pow(popBal_.continuousTurbulence().epsilon(), 2.0/3.0)
                )
              + C3_*continuousPhase.thermo().mu()
               /(
                    sqrt(continuousPhase.rho()*popBal_.refPhase().rho())
