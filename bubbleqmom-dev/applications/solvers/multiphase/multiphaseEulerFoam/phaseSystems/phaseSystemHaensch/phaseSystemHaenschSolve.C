...
void Foam::phaseSystemHaensch::solve
(
...
)
{
    ...
    for (int acorr=0; acorr<nAlphaCorr; acorr++)
    {
        ...
        )
        {
			...
			*gamma_[phase.name()] =
			    (0.5*tanh(haensch_ab_*(alpha-haensch_alphaMin_[phase.name()]))+0.5)
			    *(0.5*tanh(haensch_ab_*(haensch_alphaMax_[phase.name()]-alpha))+0.5);
                        ...
                        if (cAlpha != cAlphas_.end())
                        {
                            ...
                            phir +=
                               ...
                               *nHatf(alpha, alpha2)
				*fvc::interpolate(*gamma_[phase.name()]);
                        }
