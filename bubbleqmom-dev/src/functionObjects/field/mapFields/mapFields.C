...
)
{
    ...
    const word mapRegionName(word(dict.lookup("mapRegion")));
    ...
    const word mapMethodName(word(dict.lookup("mapMethod")));
    ...
    word patchMapMethodName = word(meshToMesh::interpolationMethodAMI(mapMethod));
    ...
    if (Switch(dict.lookup("consistent")))
    {
        interpPtr_.reset
        (
            new meshToMesh
    ...
    else
    {
        ...
        dict.lookup("patchMap") >> patchMap;
        dict.lookup("cuttingPatches") >> cuttingPatches;
