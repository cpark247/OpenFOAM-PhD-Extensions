...
bool Foam::functionObjects::stabilityBlendingFactor::init(bool first)
{
    if (residuals_)
    {
        if (mesh_.foundObject<IOField<scalar>>(residualName_))
        {
	    const auto& residual = mesh_.lookupObject<IOField<scalar>>(residualName_);
    ...
    {
	const volScalarField& nonOrth = mesh_.lookupObject<volScalarField>(nonOrthogonalityName_);
        ...
        if (first)
        {
            Log << "    Max non-orthogonality :  " << max(nonOrth).value()
                << endl;
        }
    }
    ...
    if (skewness_)
    {
	const volScalarField& skewness = mesh_.lookupObject<volScalarField>(skewnessName_);
        ...
        if (first)
        {
            Log << "    Max skewness :  " << max(skewness).value()
                << endl;
        }
    }
    ...
    if (faceWeight_)
    {
	const volScalarField& faceWeights = mesh_.lookupObject<volScalarField>(faceWeightName_);
        ...
        if (first)
        {
            Log << "    Min face weight:  " << min(faceWeights).value()
                << endl;
        }
    }
    ...
    if (gradCc_)
    {
        ...
        (
            ...
            mesh_,
            dimensionedScalar("magGradCC",dimless, Zero)
        );
        ...
        for (direction i=0; i<vector::nComponents; i++)
        {
            ...
            (
                ...
                mesh_,
                dimensionedScalar("cc" + word(vector::componentNames[i]),dimLength, Zero)
            );
            cci = mesh_.C().component(i);
            cci.correctBoundaryConditions();
            magGradCC += mag(fvc::grad(cci));
        }

        if (first)
        {
            Log << "    Max magGradCc :  " << max(magGradCC).value()
                << endl;
        }
    ...
    if (Co_)
    {
	const volVectorField& UName = mesh_.lookupObject<volVectorField>(UName_);
        ...
        (
            ...
            mesh_,
            dimensionedScalar("Co",dimless, Zero)
        );

        Co.primitiveFieldRef() =
            mesh_.time().deltaT()*mag(UName)/cbrt(mesh_.V());
	...
	const volScalarField& alpha = mesh_.lookupObject<volScalarField>(alphaName_);
        ...
        //{
        //    FatalErrorInFunction
        //        << "    alphaMax should be larger than alphaMin."
        //        << exit(FatalError);
        //}
        ...
        indicator_ =
            ...
            //    indicator_,
		min(max(scalar(0), 
		(0.5*tanh(aB_*(alpha-alphaMin_))+0.5)
		//*(0.5*tanh(aB_*(alphaMax_-alpha))+0.5)
		), scalar(1));
            //);
    }
...
:
    ...
    (
        ...
        mesh_,
        dimensionedScalar("blendedIndicator" + fieldName_,dimless, Zero)
    ),
    ...
    alpha_(dict.lookupOrDefault<Switch>("switchAlpha", false)),
    ...
    //alphaMax_(dict.lookupOrDefault<scalar>("alphaMax", 0.9)),
    aB_(dict.lookupOrDefault<scalar>("alphaMin", 100)),
...
{
    read(dict);
    tmp<surfaceScalarField> faceBlended
    (
	...
	(
        ...
        mesh_,
        dimensionedScalar(resultName_,dimless, Zero)
	)
    );
    store(resultName_, faceBlended);

    if (nonOrthogonality_)
    {
        if (!mesh_.foundObject<volScalarField>(nonOrthogonalityName_))
        {
    ...
    if (faceWeight_)
    {
        if (!mesh_.foundObject<volScalarField>(faceWeightName_))
        {
            IOobject fieldHeader
            (
                faceWeightName_,
    ...
    if (skewness_)
    {
        if (!mesh_.foundObject<volScalarField>(skewnessName_))
        {
            IOobject fieldHeader
            (
                skewnessName_,
...
)
{
    if (fieldExpression::read(dict))
    {
        ...
        dict.lookup("switchAlpha")>>alpha_;
        ...
        //dict.readIfPresent("alphaMax", alphaMax_);
        ...
        dict.readIfPresent("alphaMin", alphaMin_);


        dict.readIfPresent("aB", aB_);
	...
	if (alpha_)
	{
	    Info<<"	Including alpha with parameters: "
		//<< alphaMin_ << " and "<< alphaMax_<<" and "<<aB_<<endl;
		<< alphaMin_ << " and "<<aB_<<endl;
	}
