...
namespace Foam
{
defineTypeNameAndDebug(meshStructureGI, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

bool Foam::meshStructureGI::isStructuredCell
(
    const polyMesh& mesh,
    const label layerI,
    const label celli
...
void Foam::meshStructureGI::correct
(
    const polyMesh& mesh,
...
)
{
...

    {
        ...
        forAll(pp, patchFacei)
        {
            ...
            patchData[patchFacei] = topoDistanceData(globalFaces.toGlobal(patchFacei), 0);
        }
...

    {
        ...
        forAll(pp.meshPoints(), patchPointi)
        {
            ...
            patchData[patchPointi] = pointTopoDistanceData(globalPoints.toGlobal(patchPointi), 0);
        }
...
Foam::meshStructureGI::meshStructureGI
(
    const polyMesh& mesh,
    const uindirectPrimitivePatch& pp,
    const globalIndex& globalFaces,
