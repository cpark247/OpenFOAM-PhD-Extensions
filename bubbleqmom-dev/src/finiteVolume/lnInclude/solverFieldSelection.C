...
bool Foam::functionObjects::solverFieldSelection::updateSelection()
{
    ...
    forAll (*this,fi)
    {
        forAll (solvedFieldNames, solvedFieldi)
        {
            if ((*this)[fi].name().match(solvedFieldNames[solvedFieldi]))
            {
                newSelection.append
                (
                    fieldInfo(wordRe(solvedFieldNames[solvedFieldi]), (*this)[fi].component())
                );
                (*this)[fi].found() = true;
            }
        }
