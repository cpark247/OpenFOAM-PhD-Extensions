...
#include "periodicTimeVaryingMappedFvPatchField.H"
#include "Time.H"
...
template<class Type>
Foam::fileName
Foam::periodicTimeVaryingMappedFvPatchField<Type>::findFieldFile
(
    const word& timeName
) const
{
    ...
    // Park: Addition - Check for sampleSurface directory structure
    const fileName vectorFieldFileName
    (
        dataDir_/timeName/sampleName_/"vectorField"/fieldTableName_
    );
    
    const fileName scalarFieldFileName
    (
        dataDir_/timeName/sampleName_/"scalarField"/fieldTableName_
    );
    ...
    else if (exists(vectorFieldFileName))  // Park: Addition
    {
        return vectorFieldFileName;
    }
    else if (exists(scalarFieldFileName))  // Park: Addition
    {
        return scalarFieldFileName;
    }
    else
    {
        FatalErrorInFunction
            ...
            << " " << vectorFieldFileName << " " << scalarFieldFileName  // Park: Modified
            << exit(FatalError);
...
template<class Type>
void Foam::periodicTimeVaryingMappedFvPatchField<Type>::checkTable()
{
    // Initialise
    if (mapperPtr_.empty())
    {
        ...
        if (debug)
        {
            Info<< "periodicTimeVaryingMappedFvPatchField :"
        ...
        if (debug)
        {
            Info<< "periodicTimeVaryingMappedFvPatchField : In directory "
    ...
    // Park: Fixed time wrapping logic
    scalar t = time().value();
    scalar wrappedTime;
    ...
    else
    {
        ...
        wrappedTime = std::fmod(t, timeLimit_);
        if (wrappedTime < SMALL)
        {
            wrappedTime = timeLimit_;
        }
    }
    
    if (debug)
    {
        Info << "t = " << t << ", timeLimit = " << timeLimit_ 
             << ", wrappedTime = " << wrappedTime << endl;
    }

    // Park: Direct search through sample times instead of using findTime
    for (label i = 0; i < sampleTimes_.size(); ++i)
    {
        if (sampleTimes_[i].value() >= wrappedTime - SMALL)
        {
            if (i == 0)
            {
                lo = 0;
                hi = (sampleTimes_.size() > 1) ? 1 : -1;
            }
            else
            {
                lo = i - 1;
    ...
    // If not found, use last time
    if (lo == -1 && sampleTimes_.size() > 0)
    {
        lo = sampleTimes_.size() - 1;
        hi = -1;
    }
    ...
    {
        FatalErrorInFunction
            << "Cannot find starting sampling values for wrapped time "
            << wrappedTime << " (original time: " << t << ")" << nl
...
template<class Type>
Foam::periodicTimeVaryingMappedFvPatchField<Type>::periodicTimeVaryingMappedFvPatchField
(
    const fvPatch& p,
...
:
    ...
    perturb_(dict.lookupOrDefault("perturb", 0.0)),  // Park: Changed default from 1e-5 to 0
    mapMethod_
    (
        dict.lookupOrDefault<word>
        (
            "mapMethod",
            "nearest"  // Park: Changed default from "planarInterpolation" to "nearest"
        )
    ),
    mapperPtr_(nullptr),
    sampleTimes_(0),
    timeLimit_(readScalar(dict.lookup("timeLimit"))), // Park: Addition
    startSampleTime_(-1),
    startSampledValues_(0),
    startAverage_(Zero),
    endSampleTime_(-1),
...
template<class Type>
Foam::periodicTimeVaryingMappedFvPatchField<Type>::
periodicTimeVaryingMappedFvPatchField
(
    const periodicTimeVaryingMappedFvPatchField<Type>& ptf
)
:
    patch_(ptf.patch_),
    ...
    timeLimit_(ptf.timeLimit_), // Park: Addition
...
template<class Type>
void Foam::periodicTimeVaryingMappedFvPatchField<Type>::autoMap
(
...
template<class Type>
void Foam::periodicTimeVaryingMappedFvPatchField<Type>::rmap
(
    const periodicTimeVaryingMappedFvPatchField<Type>& tiptf,
...
template<class Type>
Foam::tmp<Foam::Field<Type>> Foam::periodicTimeVaryingMappedFvPatchField<Type>::map()
{
    checkTable();

    // Park: Calculate wrapped time for interpolation weight
    scalar t = time().value();
    scalar wrappedTime;
    ...
    if (endSampleTime_ == -1)
    {
        ...
        if (debug)
        {
            Pout<< "map : Sampled, non-interpolated values"
                << " from start time:"
    ...
    else
    {
        ...
        const scalar s = (wrappedTime - start)/(end - start);  // Park: Use wrapped time

        if (debug)
        {
            Pout<< "map : Sampled, interpolated values"
                << " between start time:"
    ...
    // Enforce average. Either by scaling (if scaling factor > 0.5) or by offsetting.
    if (setAverage_)
    {
    ...
    if (debug)
    {
        Pout<< "map : set fixedValue to min:" << gMin(fld)
            << " max:" << gMax(fld)
...
template<class Type>
void Foam::periodicTimeVaryingMappedFvPatchField<Type>::write
(
    Ostream& os
) const
{
    ...
    writeEntry(os, "timeLimit", timeLimit_);
